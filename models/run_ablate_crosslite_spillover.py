...
STRIDE = 8
SETTINGS = [
    ("on",  False),  # cross enabled
    ("off", True),   # cross disabled
]
...
def main():
    ...
    for sleeve, tickers in SLEEVES.items():
        ...
        for name, disable in SETTINGS:
            tag = f"{sleeve}_L{L}_H{H}_crosslite_{name}_p{PATCH}_s{STRIDE}"
            cmd = [sys.executable, "-m", "src.train_baselines",
                   "--npz", str(npz),
                   "--encoder", "cross",
                   "--patch-len", str(PATCH), "--stride", str(STRIDE),
                   "--d-model", "256", "--heads", "8", "--depth", "3",
                   "--dropout", "0.1", "--epochs", "3",
                   "--eval-test", "--dump-preds",
                   "--tag", tag]
            if disable:
                cmd.append("--cross-disable")
