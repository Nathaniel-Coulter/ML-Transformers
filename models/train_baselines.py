...
def main():
    ...
    ap.add_argument("--dump-preds", action="store_true",
                help="Save per-date test predictions, truths, and dates for regime/cost eval")
    ...
    # Optional date arrays (only present if loader saved them)
    dates_tr = npz["dates_train"] if "dates_train" in npz.files else None
    dates_va = npz["dates_val"]   if "dates_val"   in npz.files else None
    dates_te = npz["dates_test"]  if "dates_test"  in npz.files else None
    ...
    if args.eval_test:
        ...
        tag_str = f"{args.tag}" if args.tag else f"{args.encoder}_L{L}_H1"
        print(f"[RESULT] tag={tag_str} test_mse={mse:.6f} test_mae={mae:.6f} sharpe={sharpe:.3f} turnover={turnover:.3f}")

    # If requested, dump per-date test arrays for regime/cost analysis
    if args.dump_preds:
        os.makedirs("outputs/preds", exist_ok=True)
        np.save(f"outputs/preds/{tag_str}_y_true.npy", y_true_np)
        np.save(f"outputs/preds/{tag_str}_y_pred.npy", y_pred_np)
        if dates_te is not None:
            # ensure these are strings (ISO) for robust alignment later
            np.save(f"outputs/preds/{tag_str}_dates.npy", np.asarray(dates_te))
        print(f"[save] preds -> outputs/preds/{tag_str}_{{y_true,y_pred,dates}}.npy")
